...
- Can't yet verify via the public API that an older `RecipeVersion`'s data stays untouched when a new version is added — `RecipeResponse` only exposes `CurrentVersion`. Not a known bug, just an untested gap; would need a `GetRecipeVersions`-style endpoint or a direct DB check to confirm.
